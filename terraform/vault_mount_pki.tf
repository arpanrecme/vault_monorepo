...
resource "tls_locally_signed_cert" "pki_intermediate_cert" {
  ...
  ca_private_key_pem = file(var.VAULT_MONO_LOCAL_FILE_TERRAFORM_INTERMITTENT_CA_PRIVATE_KEY)
  ca_cert_pem        = file(var.VAULT_MONO_LOCAL_FILE_TERRAFORM_INTERMITTENT_CA_CERTIFICATE)
...
resource "vault_pki_secret_backend_intermediate_set_signed" "certificate" {
  ...
  file(var.VAULT_MONO_LOCAL_FILE_TERRAFORM_INTERMITTENT_CA_CERTIFICATE))
}
